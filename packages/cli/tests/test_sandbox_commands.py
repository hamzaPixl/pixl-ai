...
class TestSandboxList:
    def test_list_empty(self, runner, mock_db):
        ...
        with patch(
            "pixl_cli.context.CLIContext.db", new_callable=lambda: property(lambda self: mock_db)
        ):
    ...
    def test_create_with_repo(self, runner, mock_client, mock_db):
        ...
        with (
            ...
            result = runner.invoke(
                cli, ["sandbox", "create", "proj-1", "--repo-url", "https://github.com/test/repo"]
            )
        call_args = mock_client.create.call_args
        assert (
            call_args[1].get("repo_url") == "https://github.com/test/repo" or call_args[0][1]
