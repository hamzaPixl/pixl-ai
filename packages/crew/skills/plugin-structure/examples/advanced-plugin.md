...
A complex, enterprise-grade plugin with advanced organization.
...
2. **Trigger**: Start build pipeline
   \`\`\`bash
   gh workflow run build.yml --ref "$currentBranch"
   \`\`\`
...
- **Performance**: Parallel operations for speed
